...
import { MovesAssistantCapabilitiesService } from '../../../services/moves-assistant-capabilities.service';
...
/**
 * Moves' own assistant-box - trimmed port of TODD's
 * shared/page/assistant-box/assistant-box.component.ts, same scoping
 * decision as web-products/network and web-products/pulse's ports. No
 * local-task-intent pre-processing layer (TODD's original never had one
 * for tasks any more than it did for surveys), so prompts go straight from
 * direct-nav/pre-checks to the Task LLM.
 */
@Component( {
...
export class AssistantBoxComponent implements OnInit, OnChanges, OnDestroy {
    ...
    assistantResponse = '';
    isLoading = false;
    @Input() placeholder = 'Ask about a move, or tell me what you need.';
    PRODUCTION = false;
    ...
    constructor (
        ...
        private capabilities: MovesAssistantCapabilitiesService,
    ...
    private buildContextSignature (): string | null {
        ...
        const signature = {
            ...
            summary: {
                totalMoveCount: summary['totalMoveCount'] ?? summary['totalMoves'] ?? null,
            }
        };
    ...
    async askAssistant (): Promise<void> {
        ...
        const helpers: AssistantEngineHelpers = {
            ...
            routeDomain: () => 'task',
            handleTaskLLMIntent: ( raw: string ) => {
                this.openAISubscription?.unsubscribe?.();
                ...
                this.openAISubscription = this.taskLLM.run( {
                    ...
                    onError: ( err ) => { this.isLoading = false; patches.setAssistantResponse( this.formatAssistantError( err, 'moves' ) ); }
                } );
            },
            buildUiHint: () => '',
