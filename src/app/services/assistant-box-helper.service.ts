 ...
 * backtick-stripping term list are cut down to Moves' own routes only -
 * same trim as web-products/network and web-products/pulse's ports.
 */
...
export class AssistantBoxHelperService {
  ...
  private routeLabelMap: Record<string, string> = {
    '/app': 'Execution',
    '/moves-view': 'Moves Browser',
    '/move': 'Create Move',
    '/ai-missions': 'AI Missions',
    '/plan': 'Mission Workspace',
    '/pricing': 'Pricing',
    '/help': 'Help',
  };
  ...
  public stripBackticksAroundRoutes ( s: string ): string {
    ...
    const terms = ['Moves', 'Moves Browser', 'Execution', 'AI Missions', 'Mission Workspace', 'Home'];
  ...
  public tryWhatWorkIntent ( p: string ): string | null {
    if ( /\b(what work do you do|what do you actually do|do you actually do work|what work you do)\b/i.test( p ) ) {
      ...
      <ul>
        <li>Track moves, deadlines, and who owns what</li>
        <li>Flag overdue or stale moves before they slip</li>
        <li>Surface disconnected moves with no mission or contact attached</li>
        <li>Help you create a new move from a plain-English description</li>
      </ul>
    `);
    }
    return null;
