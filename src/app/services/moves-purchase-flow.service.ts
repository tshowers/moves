...
import { MovesEntitlementService } from './moves-entitlement.service';
...
/**
 * Trimmed from services/purchase-flow.service.ts, ported via Network's
 * NetworkPurchaseFlowService - same checkout/confirm calls against the
 * same todd-backend endpoints (nothing about billing changes), but drops
 * MarketingFunnelService's trackCheckoutStarted/trackPaidConversion calls,
 * which are fire-and-forget marketing-funnel analytics, not core to
 * completing a purchase - and MarketingFunnelService itself pulls in
 * TODD's full AuthService/UserService, which isn't worth carrying over
 * for an analytics side-effect.
 */
@Injectable( {
  providedIn: 'root'
} )
export class MovesPurchaseFlowService {
  constructor ( private entitlementService: MovesEntitlementService ) {}
