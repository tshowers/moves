...
export interface AssistantEngineHelpers {
    ...
    handleTaskLLMIntent: ( prompt: string ) => void;
 ...
 * web-products/network and web-products/pulse's ports (direct nav ->
 * pre-checks -> domain routing -> general LLM fallback).
 */
...
export class AssistantEngineService {
    async run (
        ...
        if ( helpers.routeDomain( rawClean ) === 'task' ) {
            patches.setAssistantPrompt( '' );
            io.scheduleScrollToBottom( true );
            helpers.handleTaskLLMIntent( rawClean );
            return;
        }
