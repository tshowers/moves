...
/**
 * Ported from services/entitlement.service.ts via Network's
 * NetworkEntitlementService - logic unchanged, only the auth dependency is
 * swapped for MovesAuthService. This is generic infrastructure (used by
 * all 6 of TODD's paid modules, not Moves-specific), so it's a
 * near-verbatim port rather than a trimmed reimplementation.
 */
...
export class MovesEntitlementService {
